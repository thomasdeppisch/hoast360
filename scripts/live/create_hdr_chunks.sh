...
ffmpeg -y -re -i '../videos/hoast_analyzer.mp4' -i '../sounds/wavs/hoast_demo.wav' \
    ...
    -f webm_chunk -audio_chunk_duration 5000 -header "./media/audio_171.hdr" -chunk_start_index 1 "./media/audio_171_%d.chk"

# always make sure that -keyint_min and -g are set to the same value
# and that audio_chunk_duration = keyint_min / fps